...
df = pd.read_csv('results_by_features.csv')
...
p1, = host.plot(df['Number of features'], df['Accuracy'], label="Accuracy")
p4, = host.plot(df['Number of features'], df['f1-score'], label="f1-score")
p5, = host.plot(df['Number of features'], df['MCC'], label="MCC")


p2, = par1.plot(df['Number of features'], df['Training time'], label="Training time")
p3, = par2.plot(df['Number of features'], df['Energy consumption'], label="Energy consumption")

host.set(xlabel="Number of features", ylabel="Value")
